...
<div class="col-2 h-100 d-flex flex-column justify-content-between">
    <div>
        <div class="py-4 k2d ps-3">
            <a class="text-light fs-4 link-underline link-underline-opacity-0 d-flex align-items-center" href="/user/dashboard">
        ...
        <nav class="nav fs-5 fw-light d-flex flex-column">
            {foreach $navs as $nav}
                <a class="nav-link me-2 px-0 text-gray" href="/user/{$nav[0]}">
                    {if strpos($smarty.server.REQUEST_URI, "/user/{$nav[0]}") !== false || ( $nav[0] === "dashboard" && $smarty.server.REQUEST_URI === "/user") }
